...
export const TourEditor: React.FC = () => {
  ...
  const methods = useForm({
    defaultValues: tour || {
      ...
      itineraries: [], pricing_packages: [], gallery: [], highlights: [], inclusions: [], faqs: []
    }
  });
  ...
  const onSubmit = async (data: any) => {
    // Mutation logic here (via useTourForm hooks)
    console.log("Saving massive tour object:", data);
    alert("Check console for the comprehensive normalized payload!");
  };
      ...
      <form onSubmit={methods.handleSubmit(onSubmit)} className="max-w-6xl mx-auto pb-20">
        <div className="flex justify-between items-center mb-8 sticky top-0 bg-slate-50/90 backdrop-blur-md py-4 z-50">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-black text-slate-900">{id ? 'Edit Expedition' : 'New Expedition'}</h1>
          </div>
          <button 
            type="submit"
            className="bg-emerald-600 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-emerald-700 shadow-xl shadow-emerald-100 transition-all"
          >
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex border-b border-slate-100 bg-slate-50/50 overflow-x-auto no-scrollbar">
            ...
            <TabButton active={activeTab === 'content'} onClick={() => setActiveTab('content')} label="Content" icon={List} />
            ...
            <TabButton active={activeTab === 'policy'} onClick={() => setActiveTab('policy')} label="Policy" icon={ShieldCheck} />
          </div>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <input {...methods.register('title')} className="w-full p-4 bg-slate-50 border rounded-xl" placeholder="e.g. Grand Canyon Sunset Trek" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Slug</label>
                    <input {...methods.register('slug')} className="w-full p-4 bg-slate-50 border rounded-xl" placeholder="grand-canyon-sunset" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-2">
            ...
            {activeTab === 'gallery' && (
              <div className="space-y-6">
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <label className="aspect-video border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-slate-50 transition-all">
                    <input 
                      type="file" 
                      className="hidden" 
                      onChange={(e) => handleImageUpload(e, (url) => appendGallery({ image_url: url, sort_order: gallery.length }))}
                    />
                    {isUploading ? <Loader2 className="animate-spin text-emerald-600" /> : <Upload className="text-slate-400" />}
                    <span className="text-[10px] font-bold text-slate-400 uppercase">Add Image</span>
                  </label>
                </div>
              </div>
            )}
            ...
            {activeTab === 'itinerary' && (
              <div className="space-y-6">
                {itineraries.map((field, index) => (
                  <div key={field.id} className="p-6 bg-slate-50 border rounded-xl relative">
                    <button onClick={() => removeItinerary(index)} className="absolute top-4 right-4 text-slate-300 hover:text-rose-500">
                      <Trash2 className="w-5 h-5" />
                    </button>
                    <div className="grid grid-cols-12 gap-6">
                      <div className="col-span-1">
                        <input {...methods.register(`itineraries.${index}.day_number`)} type="number" className="w-full p-2 border rounded-lg text-center font-bold" placeholder="Day" />
                      </div>
                      <div className="col-span-11 space-y-4">
                        <input {...methods.register(`itineraries.${index}.title`)} className="w-full p-3 border rounded-xl font-bold" placeholder="Segment Title" />
                        <textarea {...methods.register(`itineraries.${index}.description`)} className="w-full p-3 border rounded-xl text-sm" placeholder="Detailed description..." rows={3} />
                      </div>
                    </div>
                  </div>
                ))}
                <button 
                  ...
                  className="w-full py-4 border-2 border-dashed rounded-xl flex items-center justify-center gap-2 text-slate-400 font-bold hover:bg-emerald-50 hover:text-emerald-600 transition-all"
                >
                  <Plus className="w-5 h-5" /> Add Itinerary Item
                </button>
              </div>
            )}
            
            {/* ... Other tabs (Pricing, Policy, Content) implemented with same useFieldArray pattern ... */}
          </div>
        </div>
      </form>
    </FormProvider>
