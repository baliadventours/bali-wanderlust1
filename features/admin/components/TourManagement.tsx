...
import { Plus, Edit3, Trash2, MapPin, Search, Filter } from 'lucide-react';
...
export const TourManagement: React.FC = () => {
  ...
  if (isLoading) return <div className="p-10 text-center"><p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Syncing Inventory...</p></div>;
        ...
        {filtered?.map((tour) => (
          <div key={tour.id} className="bg-white rounded-[10px] border border-slate-200 overflow-hidden group hover:border-emerald-200 transition-all">
            <div className="aspect-video relative overflow-hidden bg-slate-100">
              ...
              <div className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest ${tour.status === 'published' ? 'bg-emerald-500 text-white' : 'bg-slate-400 text-white'}`}>
