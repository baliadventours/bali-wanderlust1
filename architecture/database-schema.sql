...
-- Allow users to insert their own profile as a fallback if the trigger is slow
CREATE POLICY "Users Insert Own Profile" ON public.profiles FOR INSERT WITH CHECK (auth.uid() = id);

-- Explicitly split Tour Manage policy to be more robust
CREATE POLICY "Public Read Tours" ON public.tours FOR SELECT USING (true);
CREATE POLICY "Admin Insert Tours" ON public.tours FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);
CREATE POLICY "Admin Update Tours" ON public.tours FOR UPDATE USING (
    EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);
CREATE POLICY "Admin Delete Tours" ON public.tours FOR DELETE USING (
    EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);
...
BEGIN
  ...
  VALUES (
    ...
    new.email, 
    'admin' -- ALL users are admins for this development phase
  )
